// compile:
//use this:         g++ -std=c++11 *.cpp -o mpc-gui-p2p-peer3 `pkg-config gtkmm-3.0 --cflags --libs`
...
int main(int argc, char *argv[]) {
	...
	//--------------------------------------------------- GUI without Glade //start
	  //~ auto app = Gtk::Application::create(argc, argv, "org.gtkmm.example3");

	  //~ //MPCGUI window;
	  //~ MPCGUI window(3);  // 3 means player 3
	...
	MPC_Player myPlayer(3, 2); // 1 means player 1
	myPlayer.startMPCConsoleMode();
	//--------------------------------------------------- MPCConsoleMode //end
