// compile:
//use this:         g++ -std=c++11 *.cpp -o mpc-gui-p2p-peer4 `pkg-config gtkmm-3.0 --cflags --libs`
...
int main(int argc, char *argv[]) {
	...
	//--------------------------------------------------- GUI without Glade //start
	  //~ auto app = Gtk::Application::create(argc, argv, "org.gtkmm.example4");

	  //~ //MPCGUI window;
	  //~ MPCGUI window(4);  // 4 means player 4
	...
	MPC_Player myPlayer(4, 2); // 1 means player 1
	myPlayer.startMPCConsoleMode();
	//--------------------------------------------------- MPCConsoleMode //end
